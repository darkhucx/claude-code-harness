...
argument-hint: "[code|plan|scope] [--dual] [--gemini] [--security] [--ui-rubric]"
context: fork
effort: high
...
├── --dual が含まれる        → Dual Review モード (Codex 並行) → Step 3.5 へ
├── --gemini が含まれる      → Gemini Review モード (Gemini 並行) → Step 3.5b へ
...
### Step 3.5b: --gemini フラグ時の Gemini 並行レビュー

`--gemini` フラグが指定されている場合、Step 3 の Claude レビューと並行して Gemini レビューを実行し、結果をマージする。
Step 3.5 の Codex 並行レビューと 1:1 対応した操作体系で、`sakibsadmanshajib/gemini-plugin-cc` の ACP (Agent Client Protocol) 経由で動作する。

**重要**: Gemini plugin の `review` subcommand は schema を適用せず free-text を返すため、構造化 verdict を取る場合は `adversarial-review --background` を使う（同じ `review-output.schema.json` を適用する経路）。前台 `review` は ACP 初期化タイミング問題で失敗することがあり、`--background` が唯一の安定経路。

1. Gemini の利用可否を確認する（`scripts/gemini-companion.sh setup --json` の `geminiAvailable` と `authenticated` を確認）
2. 利用可能であれば background で adversarial-review を起動:
   ```bash
   JOB_OUT=$(bash scripts/gemini-companion.sh adversarial-review --base "${BASE_REF:-HEAD~1}" --background)
   JOB_ID=$(echo "$JOB_OUT" | grep -oE 'gemini-[0-9]+-[a-z0-9]+' | head -1)
   ```
3. 完了まで `status` を poll する（平均 2〜4 分）:
   ```bash
   ...
   ```
4. 結果を取得し、rawOutput 内の ```json fence を剥がして schema 形式に正規化:
   ```bash
   ...
   ```
5. `write-review-result.sh` に渡す（Codex と同じ normalize 経路。`needs-attention → REQUEST_CHANGES` / `approve → APPROVE`）:
   ```bash
   bash scripts/write-review-result.sh /tmp/gemini-clean.json "${COMMIT_HASH}"
   ```
6. 最終レビュー結果に `dual_review` フィールドを付加し、`dual_review.engine = "gemini"` と `dual_review.session_id = "..."` を明示する

**Codex 並行との差異**:

- 認証: `gemini auth` (OAuth) または `GEMINI_API_KEY` 環境変数
- モデル: 既定は `auto-gemini-3`（Gemini 3.x 系に自動ルーティング）
- subcommand: Gemini は `adversarial-review`（schema 適用のため）、Codex は `review` でよい
- 実行経路: Gemini は必ず `--background` + poll + result、Codex は前台実行可能
- 中間抽出ステップ: Gemini は `gemini-review-extract.sh` で `rawOutput` 内の JSON fence を剥がす必要がある（plugin が background worker 経路で schema 出力を raw text に包むため）
- findings severity 欠落の fallback: Gemini 出力は schema 準拠率が低く、`severity` が欠落することがある。`gemini-review-extract.sh` が `severity: "medium"` で埋める（安全側、followups に分類される）
- `--dual` との**同時指定は不可**。どちらか一方だけ使う（両エンジンに投げたい場合は連続 2 回実行）

フォールバック: `gemini-companion.sh setup --json` が `geminiAvailable=false` または `authenticated=false` を返す場合は Claude 単独レビュー（Step 3）へフォールバックし、警告メッセージを出力する。ジョブが `status=failed` で終わった場合も同じフォールバック経路。
...
- Harness flow 内のレビューは `codex-companion.sh review` / `gemini-companion.sh review`（優先）と `reviewer` agent
