# Plans Archive — Phase 37 + 41 + 42 + 43

Archived: 2026-04-17
Reason: 全タスク cc:完了 のため Plans.md から切り出し（行数削減）
Origin: main branch commit 158e4e05 時点
Archive file: .claude/memory/archive/Plans-2026-04-17-phase37-41-42-43.md
...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 43.3.1 | `scripts/codex-loop.sh`, `skills/harness-loop/SKILL.md`, `skills/harness-loop/references/flow.md`, `skills-codex/harness-loop/SKILL.md` を更新し、advisor consult を loop に導入する。相談点は (a) 高リスク task の初回実行前、(b) 同一原因の 2 回目失敗後、(c) `PIVOT_REQUIRED` の直前の 3 箇所に固定する。`trigger_hash = task_id + reason_code + normalized_error_signature` を使い、同じ trigger では 1 回しか相談しない | (a) 同一 task で同一 trigger を 2 回連続で相談しない、(b) `max_consults=3` 超過でユーザーエスカレーションに移る、(c) `status --json` に `consultations`, `last_decision`, `last_trigger`, `last_model` が出る | 43.2.2 | cc:完了 [e159a45] |
| 43.3.2 | loop 内の advisor decision 処理を固定する。`PLAN` は次回 executor prompt 先頭に advice を prepend して再実行、`CORRECTION` は修正指示として再実行、`STOP` は loop を止めて state と runner.log に理由を残す。`plateau` 判定は置き換えず、`PIVOT_REQUIRED` のときに advisor を 1 回だけ挟んでから停止判断する | (a) `PLAN / CORRECTION / STOP` それぞれで loop の遷移がテストで固定される、(b) plateau 前相談が 1 回だけ入る、(c) 既存の review / checkpoint / Plans 更新フローに regressions なし | 43.3.1 | cc:完了 [e159a45] |
...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 43.4.1 | `skills/breezing/SKILL.md`, `skills/harness-work/SKILL.md`, `skills-codex/breezing/SKILL.md`, `skills-codex/harness-work/SKILL.md` と関連 flow を更新し、Worker が advice request を返したときだけ Lead が advisor を呼び、同じ Worker に `SendMessage` / `send_input` で返す流れを追加する。Advisor は `Reviewer` より前にだけ動き、品質判定は Reviewer に残す | (a) Worker → Lead → Advisor → Worker の protocol が文書と擬似コードで一致、(b) `Reviewer` の APPROVE/REQUEST_CHANGES は従来どおり最終成果にのみ適用、(c) skill mirror の整合テストが PASS | 43.3.2 | cc:完了 [e159a45] |
...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 43.5.1 | `harness-work` の solo 実行で parent session 自身が Lead を兼ねる前提の advisor consult フローを追加する。相談条件、相談 budget、decision 処理は loop と同じにし、`Worker` 不在の場面でも `advisor-request.v1` / `advisor-response.v1` を使うように flow を統一する | (a) solo 実行の文書と flow が loop/breezing と同じ trigger 条件を使う, (b) `STOP` は即エスカレーション、(c) review artifact の gate を飛ばさない | 43.4.1 | cc:完了 [e159a45] |
