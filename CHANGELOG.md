...
### Fork addition (darkhucx): 中国語 i18n 対応 (originally Phase 62)
...
### Fork addition (darkhucx): Codex auth + Ollama 統合 (originally Phase 63/64, renumbered to 70/71)

**今まで**: Codex CLI の API key 設定は `~/.codex/config.toml` の手書きが必要。ローカル AI による小タスク処理の選択肢がなかった。

**今後**: `bash scripts/codex-companion.sh auth` で対話式に API Key を設定可能（パーミッション 600 / status / logout 完備）。`scripts/ollama-companion.sh` を新設し `harness-work --ollama` でローカル Ollama にタスク委託可能。`score-task` サブコマンドで複雑度ベースの自動ルーティングも提供。
