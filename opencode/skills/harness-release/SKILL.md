...
description-zh: "通用发布自动化技能。适用于任何使用 Keep a Changelog 和 GitHub 的项目。单一确认门之后端到端自动完成 bump 判定、CHANGELOG 升级、打 tag、GitHub Release。触发：发布、版本提升、tag 创建、公开。不用于：实现、代码评审、计划、初始化。"
...
[Pre-Gate: 情報収集のみ、ファイル未変更]
  ...
  2. Version file 自動検出
  3. 現在バージョンの読み取り
  4. [Unreleased] 内容の解析 → bump level 推定
  5. 新バージョン算出
  6. CHANGELOG 差分ドラフト作成 (メモリ上)
  7. GitHub Release notes ドラフト作成 (メモリ上)
...
[Post-Gate: 承認後、中断なし]
  ...
  8. Version file 書き換え
  9. CHANGELOG.md 書き換え ([Unreleased] → [X.Y.Z] 昇格 + compare link)
  10. git add + commit
  11. git tag -a vX.Y.Z
  12. git push origin <branch> --tags
  13. gh release create vX.Y.Z
  14. 完了報告
```
