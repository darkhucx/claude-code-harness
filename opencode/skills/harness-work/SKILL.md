...
argument-hint: "[all] [task-number|range] [--codex] [--gemini] [--parallel N] [--no-commit] [--resume id] [--breezing] [--auto-mode]"
effort: high
---
...
明示的なモードフラグ（`--parallel`, `--breezing`, `--codex`, `--gemini`）がない場合、
...
2. **`--codex` / `--gemini` は明示時のみ発動**。各 CLI が未インストールの環境があるため、自動選択しない
3. `--codex` は他モードと組み合わせ可能: `--codex --breezing` → Codex + Breezing
4. `--gemini` も他モードと組み合わせ可能: `--gemini --breezing` → Gemini + Breezing
5. `--codex` と `--gemini` は**同時指定不可**（どちらか 1 つのみ）
...
`sakibsadmanshajib/gemini-plugin-cc` の companion 経由で Gemini CLI にタスクを委託する。
Codex モードと 1:1 対応した操作体系で、ACP (Agent Client Protocol) 経由で
Job 管理・thread resume・構造化出力を提供する。

```bash
# タスク委託（書き込み可能）
bash scripts/gemini-companion.sh task --write "タスク内容"

# stdin 経由（大きなプロンプト向け）
GEMINI_PROMPT=$(mktemp /tmp/gemini-prompt-XXXXXX.md)
# タスク内容を書き出し
cat "$GEMINI_PROMPT" | bash scripts/gemini-companion.sh task --write
rm -f "$GEMINI_PROMPT"

# 前回スレッドの続行
bash scripts/gemini-companion.sh task --resume-last --write "続きをやって"

# モデル指定（既定: auto-gemini-3）
bash scripts/gemini-companion.sh task --write --model pro "タスク内容"
```

**Codex モードとの差異**:

- effort レベル: Codex の `--effort (none|minimal|low|medium|high|xhigh)` は
  Gemini 側では `--thinking (off|low|medium|high)` にマップされる
  （`gemini-companion.sh` が自動で正規化）
- モデルエイリアス: `pro | flash | flash-lite | auto-gemini-3 | auto-gemini-2.5`
- 認証: `gemini auth` (OAuth) または `GEMINI_API_KEY` 環境変数
- `--codex` との**同時指定は不可**。どちらか一方だけ使う
