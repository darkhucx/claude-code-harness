---
name: gogcli-ops
description: "通过 gogcli 进行 Google Workspace CLI 操作（Drive/Sheets/Docs/Slides）。在用户要求通过 gogcli 检查、列出、搜索、导出、读取或更新 Google 文件；需要解析 Google URL/ID；需要身份认证/账号选择或安全只读流程；或排查 gogcli 访问/错误时触发。不用于：通用文件操作、非 Google 云存储或标准 shell 命令。"
...
description-zh: "通过 gogcli 进行 Google Workspace CLI 操作（Drive/Sheets/Docs/Slides）。在用户要求通过 gogcli 检查、列出、搜索、导出、读取或更新 Google 文件；需要解析 Google URL/ID；需要身份认证/账号选择或安全只读流程；或排查 gogcli 访问/错误时触发。不用于：通用文件操作、非 Google 云存储或标准 shell 命令。"
